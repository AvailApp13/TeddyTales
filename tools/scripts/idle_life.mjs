...
const EAR_T = 30;
const EAR_UP = 0, EAR_DOWN = 24;                                         // поворот уха вокруг оси в голове, ° (D26, D27): основание тянется, капюшон стоит
...
const LISTEN = { from: 480, to: 840, head: 0, t: 160 };   // наклон головы при прислушивании выключен: ломал ровный ритм покачивания
...
// покачивание — один ровный маятник: две синусоиды (432 и 288 кадров) давали неровный ритм —
// голова то почти стояла, то быстро уходила на большой угол (замер по кадрам рантайма)
const SWAY = { amp: 4, period: 432 };                                   // ±4°, 7,2 с на полный цикл, 2 цикла в петле
const drift = (f) => SWAY.amp * S(PI2 * f / SWAY.period);
