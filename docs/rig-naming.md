...
- **Фон артборда прозрачный.** У самого артборда не должно быть заливки (в
  редакторе — «цвет фона артборда»), и фигур во весь артборд с видимой
  заливкой тоже. Фон рисует приложение, персонаж стоит поверх сцены комнаты.
  В ТЗ этого пункта нет, а в стороннем демо-риге такая заливка была и
  закрывала собой всю комнату; отключить её из кода нельзя — рантайм умеет
  только вписать артборд в область. Фигура во весь артборд с выключенной
  заливкой — законный приём (невидимая хит-зона), её линтер не трогает.
