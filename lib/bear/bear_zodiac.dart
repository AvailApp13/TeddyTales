...
///
/// Знак ставит сервер в момент регистрации — по календарной дате этого дня
/// у человека (заказчик 24.09, миграция 0011; справочник знаков —
/// `zodiac_signs`). Здесь вычисления нет: значение приходит с сервера вместе
/// с карточкой рождения.
enum BearZodiac {
  aries('Овен', '♈'),
  taurus('Телец', '♉'),
  gemini('Близнецы', '♊'),
  ...
  const BearZodiac(this.title, this.symbol);

  final String title;

  /// Знак зодиака символом — как в справочнике на сервере.
  final String symbol;
}
