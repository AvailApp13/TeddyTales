...
# Головы набора v6. Каждая — цельный снимок головы с капюшоном, ушами и
# лицом, вырезанный из своего кадра одной и той же рамкой. Все семь сведены
# на одну сетку, поэтому подмена одной на другую не двигает мишку ни на
# пиксель: меняется только выражение.
HEADS = ('calm', 'happy', 'sad', 'sleep', 'chew', 'gazeL', 'gazeR')

HEAD_LAYERS = [f'head_{name}' for name in HEADS]

# Порядок отрисовки сверху вниз: первый в списке рисуется поверх остальных.
# Риг v6: три яруса вместо девяти кусков. Головы лежат стопкой в одной точке
# и видна всегда ровно одна — остальные с нулевой прозрачностью. Голова
# поверх кофты, кофта поверх ног; на швах перекрытие 220 пикселей, так что
# при дыхании и кивке дыре взяться неоткуда.
DRAW_ORDER = ['hearts'] + HEAD_LAYERS + ['torso', 'legs']

# Что выкидываем из файла вместе с картинками. Первая волна от нейросети,
# отдельные черты лица из v3 — и весь разборный набор v4: в v6 лицо, уши и
# лапы приходят внутри цельных снимков, отдельными слоями их больше нет.
...
BREATH_LIFT = 5.0           # на сколько поднимается голова, пикселей артборда
...
MESHES = {
    ...
    # Посередине груди мёртвая зона: там планка с белой пуговицей, и сетка,
    # проходя по ней, вытягивала пуговицу в запятую — жёсткая деталь на
    # мягкой ткани сразу выдаёт подделку. Колонок семь, а не пять: чем чаще
    # сетка, тем плавнее ткань между узлами.
    'torso': {'grid': (7, 4), 'rows': (0.25, 0.9, 1.0, 0.55),
              'swell': 12.0, 'centre_dead': 0.3, 'sag': 3.0},
    # Ноги: бёдра дышат чуть-чуть, ступни приколочены к полу.
    'legs': {'grid': (5, 3), 'rows': (0.8, 0.45, 0.0),
             'swell': 4.0, 'centre_dead': 0.0, 'sag': 0.0},
}
"""Детали, которым положена сетка деформации.

Голова в этот список не входит намеренно. В v4 она тянулась сеткой, потому
что была склейкой из кусков и по ней ползали отдельные глаза и рот. В v6
голова — цельный снимок настоящей игрушки, и деформировать фотографию лица
нечем: любая волна по щеке немедленно читается как резина. Живая голова
здесь делается иначе — подъёмом на вдохе и подменой выражения.
"""
...
# Вершина вдоха у каждой части своя: сначала грудь, следом голова —
# синхронное движение читается как механика, а не как дыхание.
PEAK = {'torso': 70, 'head': 82}
...
# Отличия читаются лицом, темпом и посадкой головы: happy — бодрый цикл,
# голова выше, рот до ушей; sad — медленный цикл, голова опущена, уголки
# рта вниз.
#
# `face` — какая из семи голов видна в этом настроении. `glance` — короткий
# взгляд в сторону во втором цикле: обычный покой им и живёт, иначе мишка
# смотрит в одну точку и выглядит чучелом. Подменяется та же голова, снятая
# с поворотом, поэтому взгляд уводится по-настоящему, вместе с мордой, а не
# сдвигом зрачка.
#
# Голова ходит мало: и подъём на вдохе, и посадка настроения — единицы
# пикселей. Она лежит поверх кофты, и её растушёванный низ показывает те же
# плечи, что нарисованы на кофте под ним. Разъехались на полтора десятка
...
# Косой взгляд в обычном покое. Мишка отводит глаза дважды за полный
# покой — в разные стороны и в разных вдохах, а не два раза подряд: два
# одинаковых движения кряду сразу читаются как петля. Моменты заданы долями
# вдоха, длительность — кадрами.
GLANCE = {'at': (0.62, 1.58), 'hold': 44, 'faces': ('gazeL', 'gazeR')}

# Голова поднимается одним куском вместе с капюшоном, ушами и лицом:
# в v6 всё это — одна картинка. Стопка из семи выражений едет синхронно,
# иначе при подмене видимая голова прыгнула бы на место соседней.
HEAD_GROUP = HEAD_LAYERS
