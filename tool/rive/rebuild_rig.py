...
def emo_laugh():
    """Смех, 3,4 с: зажмурился дугами, щёки круглые — присел и прыгнул от
    радости (лапы вверх, в воздухе поджал ножки, уши и капюшон отстают),
    мягко приземлился — и пять «ха-ха»: на каждом рот распахивается во
    всю ширь, между ними — поменьше; грудь, плечи и голова вздрагивают."""
    e = Emo(204)
    g = [70, 88, 106, 124, 142]
    e.face('laugh', [(10, 180)]).blinks([8, 178])
    # между «ха» рот поменьше; на пике «ха» (a+2…a+12) — во всю ширь
    small = [(10, g[0] + 2)] + [(a + 12, b + 2) for a, b in zip(g, g[1:])] + [(g[-1] + 12, 180)]
    e.face('laugh2', small)
    ...
    e.track('hips', Emo.Y, [(12, 0), (22, 9, EO), (32, -46, EO), (38, -48), (48, 0, EIN),
                            (53, 8, EO), (62, -1.5)] + pulses(g, 5, 13, -5, -2.5) + [(176, -2)])
    for side in ('l', 'r'):
    ...
    e.track('arm_l1', Emo.R, [(22, -0.03), (32, 0.03, EO), (40, 0.025), (52, 0.0), (62, 0.02)]
            + pulses(g, 5, 13, 0.045, 0.02) + [(176, 0.02)])
    ...
    e.track('hood2', Emo.R, [(32, 0.09), (44, -0.09), (56, 0.06), (64, -0.04)]
            + pulses([a + 4 for a in g], 6, 12, -0.09, -0.04) + [(176, -0.02)])
    e.track('ear_l1', Emo.R, [(32, 0.1), (42, -0.12, EO), (54, 0.08), (62, -0.05)]
            + pulses([a + 3 for a in g], 6, 12, -0.1, -0.05))
    e.track('ear_r1', Emo.R, [(32, -0.1), (42, 0.12, EO), (54, -0.08), (62, 0.05)]
            + pulses([a + 3 for a in g], 6, 12, 0.1, 0.05))
    e.track('ear_l2', Emo.R, [(34, 0.12), (46, -0.1), (58, 0.06)] + pulses([a + 6 for a in g], 6, 12, -0.08, -0.02))
    e.track('ear_r2', Emo.R, [(34, -0.12), (46, 0.1), (58, -0.06)] + pulses([a + 6 for a in g], 6, 12, 0.08, 0.02))
    return e.build()
