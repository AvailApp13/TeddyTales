...
def emo_laugh():
    ...
    # рот по ступеням (заказчик 26.09: без рывков): в прыжке широкий, к
    # «ха-ха» прикрывается; на каждом «ха» приоткрыт → приоткрыт больше →
    # широкий (пик подбородка) → обратно, ступень — 3 кадра; в конце
    # сомкнулся в улыбку, моргнул — покой
    seq = [(10, None), (56, 'laugh_m2'), (62, 'laugh_m1')]
    for a in g:
        seq += [(a + 3, 'laugh_m2'), (a + 6, None), (a + 12, 'laugh_m2'), (a + 15, 'laugh_m1')]
    seq += [(166, 'laugh_m0')]
    e.mouths(seq, 180)
