...
def emo_laugh():
    """Смех, 3,4 с: зажмурился дугами, щёки круглые, рот широко открыт —
    присел и прыгнул от радости (лапы вверх, в воздухе поджал ножки, уши и
    капюшон отстают), мягко приземлился — и пять плавных «ха-ха»:
    подбородок, щёки, грудь, плечи и голова мягко вздрагивают."""
    e = Emo(204)
    g = [70, 88, 106, 124, 142]
    e.face('laugh', [(10, 180)]).blinks([8, 178])
    # рот один (заказчик 26.09: без рывков); на «ха» подбородок мягко
    # опускается — рот чуть шире, между — чуть уже; все кривые плавные
    e.track('chin', Emo.X, pulses(g, 8, 10, -2.2, -0.4, EI, EI) + [(176, 0)])
    e.pair('cheek', Emo.X, pulses(g, 8, 10, 1.4, 0.4, EI, EI) + [(176, 0)])
    # прыжок: присел 12–22, толчок 22–32, вершина 32–38, падение к 48,
    # приземление с приседом 48–54, выпрямился к 64
    e.track('hips', Emo.Y, [(12, 0), (22, 9, EO), (32, -46, EO), (38, -48), (48, 0, EIN),
                            (53, 8, EO), (62, -1.5)] + pulses(g, 8, 10, -5, -2.5, EI, EI) + [(176, -2)])
    for side in ('l', 'r'):
        # ноги отрываются от пола позже таза и касаются его вместе с ним
        e.track(f'leg_{side}', Emo.Y, [(24, 0), (33, -44, EO), (38, -46), (48, 0, EIN)])
        # в воздухе ножки поджаты (голени вверх), к земле — выпрямились
        e.track(f'shin_{side}', Emo.X, [(26, 0), (34, -13, EO), (40, -13), (47, 0, EIN)])
    e.track('breath', Emo.SY, [(22, -0.03), (32, 0.06, EO), (48, 0.02), (54, -0.03)]
            + pulses(g, 8, 10, 0.05, 0.015, EI, EI) + [(176, 0.02)])
    e.track('chest', Emo.R, pulses(g, 8, 10, -0.015, 0.0, EI, EI))
    e.track('head', Emo.R, [(22, 0.02), (34, -0.03, EO), (54, 0.05, BACK)]
            + pulses(g, 8, 10, 0.03, 0.05, EI, EI) + [(176, 0.045)])
    ...
    e.track('ear_l1', Emo.R, [(32, 0.1), (42, -0.12, EO), (54, 0.08), (62, -0.05)]
            + pulses([a + 3 for a in g], 8, 10, -0.1, -0.05, EI, EI))
    e.track('ear_r1', Emo.R, [(32, -0.1), (42, 0.12, EO), (54, -0.08), (62, 0.05)]
            + pulses([a + 3 for a in g], 8, 10, 0.1, 0.05, EI, EI))
    e.track('ear_l2', Emo.R, [(34, 0.12), (46, -0.1), (58, 0.06)] + pulses([a + 6 for a in g], 8, 10, -0.08, -0.02, EI, EI))
    e.track('ear_r2', Emo.R, [(34, -0.12), (46, 0.1), (58, -0.06)] + pulses([a + 6 for a in g], 8, 10, 0.08, 0.02, EI, EI))
    return e.build()
...
def emo_chew():
    """Жуёт, 2,8 с: довольно прищурился, щёки набиты — жуёт с закрытым
    ртом, как воспитанный мишка: подбородок плавно ходит вверх-вниз и чуть
    вбок, щёки надуваются по очереди, голова кивает в такт. Рот не
    меняется рывком (заказчик 26.09: нужна плавность)."""
    e = Emo(168)
    c = [18, 42, 66, 90, 114]
